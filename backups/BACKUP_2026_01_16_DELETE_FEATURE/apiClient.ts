...
export async function deleteCareEvent(eventId: number) {
    try {
        const response = await fetch(`${API_BASE}/api/care-events?id=${eventId}`, {
            method: 'DELETE'
        });
        return await response.json();
    } catch (error) {
...
// ============ CARE PROGRAM FUNCTIONS ============

export async function createCareProgram(
    plantId: string,
    username: string,
    healthScoreInitial: number,
    luxInitial: number,
    photoInitialUrl: string,
    plantName: string,
    scientificName: string
) {
    try {
        const response = await fetch(`${API_BASE}/api/care-program?action=create`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                plantId,
        ...
        return { success: false, error: 'Errore creazione programma' };
    }
}

export async function getCareProgram(plantId?: string, programId?: string) {
    try {
        const params = new URLSearchParams({ action: 'get' });
        if (plantId) params.append('plantId', plantId);
        if (programId) params.append('programId', programId);

        const response = await fetch(`${API_BASE}/api/care-program?${params.toString()}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        });
        return await response.json();
        ...
        return { success: false, error: 'Errore recupero programma', hasProgram: false };
    }
}
...
export async function completeCheckpoint(
    ...
    try {
        const response = await fetch(`${API_BASE}/api/care-program?action=checkpoint`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                checkpointId,
        ...
        return { success: false, error: 'Errore completamento checkpoint' };
    }
}
